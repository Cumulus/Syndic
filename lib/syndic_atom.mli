...
(** The [category] element conveys information about a category associated with
    ...
    - [term] is a string that identifies the category to which the entry or feed
      belongs.
      {{:http://tools.ietf.org/html/rfc4287#section-4.2.2.2} See RFC 4287 §
       4.2.2.2} - [scheme], if present, is an IRI that identifies a
      ...
      Language-Sensitive.
      {{:http://tools.ietf.org/html/rfc4287#section-4.2.2.1} See RFC 4287 §
       4.2.2.1} *)
...
(** The [generator] element's content identifies the agent used to generate a
    feed, for debugging and other purposes. - [content] is a human-readable name
    for the generating agent. - [uri], if present, SHOULD produce a
...
(** The [id] element conveys a permanent, universally unique identifier for an
    ...
    definition of "IRI" excludes relative references. Though the IRI might use a
...
type rel =
  ...
  | Link of Uri.t
      (** The URI MUST be non-empty and match either the "isegment-nz-nc" or the
          "IRI" production in {{:http://tools.ietf.org/html/rfc3987} RFC3987}.
...
(** [link] defines a reference from an entry or feed to a Web resource. See
    ...
    - [href] contains the link's IRI. The value MUST be a IRI reference,
      ...
      \- [type_media] is an advisory media type: it is a hint about the type of
      the representation that is expected to be returned when the value of the
      href attribute is dereferenced. Note that the type attribute does not
      override the actual media type returned with the representation. The value
      of [type_media], if given, MUST conform to the syntax of a MIME media
      type, {{:http://tools.ietf.org/html/rfc4287#ref-MIMEREG} MIMEREG}. See
      {{:http://tools.ietf.org/html/rfc4287#section-4.2.7.3} RFC 4287 § 4.2.7.3}.
      \- [hreflang] describes the language of the resource pointed to by the
      href attribute. When used together with the [rel=Alternate], it implies a
      translated version of the entry. The value of [hreflang] MUST be a
      language tag, {{:http://tools.ietf.org/html/rfc3066} RFC3066}. See
      {{:http://tools.ietf.org/html/rfc4287#section-4.2.7.4} RFC 4287 § 4.2.7.4}.
      \- [title] conveys human-readable information about the link. The content
      of the "title" attribute is Language-Sensitive. The value [""] means that
      no title is provided. See
      ...
      it is a hint about the content length of the representation returned when
      the IRI in the href attribute is mapped to a URI and dereferenced. Note
      that the length attribute does not override the actual content length of
      the representation as reported by the underlying protocol. See
...
(** [link uri] creates a link element.
    ...
    @param rel
      The [rel] attribute of the link. It defaults to [Alternate] since
      {{:http://tools.ietf.org/html/rfc4287#section-4.2.7.2} RFC 4287 § 4.2.7.2}
      says that
      {i if the "rel" attribute is not present, the link element MUST be
...
(** [published] is a Date construct indicating an instant in time associated
    ...
    Typically, [published] will be associated with the initial creation or first
...
(** [rights] is a Text construct that conveys information about rights held in
    and over an entry or feed. The [rights] element SHOULD NOT be used to convey
    ...
    See {{:http://tools.ietf.org/html/rfc4287#section-4.2.10} RFC 4287 § 4.2.10}
*)

type title = text_construct
(** [title] is a Text construct that conveys a human-readable title for an entry
    or feed.
...
(** If an {!entry} is copied from one feed into another feed, then the source
    ...
    For this reason, Atom Processors that are performing such aggregation SHOULD
    include at least the required feed-level Metadata fields ([id], [title], and
    [updated]) in the [source] element.
...
type mime = string
(** A MIME type that conform to the syntax of a MIME media type, but MUST NOT be
    a composite type (see Section 4.2.6 of [MIMEREG]).

    {{:http://tools.ietf.org/html/rfc4287#section-4.1.3.1} See RFC 4287 §
     4.1.3.1} *)

(** [content] either contains or links to the content of the entry. The value of
    ...
    - [Src(m, iri)] means that the content is to be found at [iri] and has MIME
      ...
      choose to ignore remote content or to present it in a different manner
      than local content. The value of [m] is advisory; that is to say, when the
...
(** [summary] is a Text construct that conveys a short summary, abstract, or
    ...
    because Atom Processors might assume there is a useful summary when there is
    none.
...
(** [feed] is the document (i.e., top-level) element of an Atom Feed Document,
    ...
    - one of the [links] SHOULD have a [rel] attribute value of [Self]. This is
      the preferred URI for retrieving Atom Feed Documents representing this
      Atom feed. - There MUST NOT be more than one element of [links] with a rel
    ...
    SHOULD be different. If an Atom Feed Document contains multiple entries with
    the same {!id}, Atom Processors MAY choose to display all of them or some
    subset of them. One typical behavior would be to display only the entry with
...
(** [parse xml] returns the feed corresponding to [xml]. Beware that [xml] is
    mutable, so when the parsing fails, one has to create a new copy of [xml] to
    use it with another function. If you retrieve [xml] from a URL, you should
    use that URL as [~xmlbase].

    Raise [Error.Expected], [Expected_Data] or [Error.Duplicate_Link] if [xml]
    is not a valid Atom document.

    @param xmlbase
      default xml:base to resolve relative URLs (of course xml:base attributes
      in the XML Atom document take precedence over this). See
      {{:http://www.w3.org/TR/xmlbase/}XML Base}.

    @param self
      the URI from where the current feed was retrieved. Giving this information
      will add an entry to [links] with [rel = Self] unless one already exists.
*)
...
(** Compare entries so that more recent dates are smaller. The date of the entry
    is taken from the [published] field, if available, or otherwise [updated] is
    used. *)
...
(** [set_self feed url] add or replace the URI in the self link of the feed. You
    can also set the [hreflang] and [length] of the self link. *)
  ...
  [> `Feed of
     ...
     | `Entry of
       ...
       | `Category of
         [> `Label of string | `Scheme of string | `Term of string ] list
       | `Content of
         [> `Data of Syndic_xml.t list | `SRC of string | `Type of string ] list
       | `Contributor of person
       | `ID of string list
       | `Link of
         [> `HREF of string
       ...
       | `Source of
         ...
         | `Contributor of person
         | `Generator of
           [> `Content of string | `URI of uri | `Version of string ] list
         | `ID of string list
