...
{{: http://web.resource.org/rss/1.0/spec#s5.3.1} See RSS 1.0 § 5.3.1 }

{{: http://web.resource.org/rss/1.0/spec#s5.4.1} See RSS 1.0 § 5.4.1 }

{{: http://web.resource.org/rss/1.0/spec#s5.5.1} See RSS 1.0 § 5.5.1 }

{{: http://web.resource.org/rss/1.0/spec#s5.6.1} See RSS 1.0 § 5.6.1 }
...
Or a brief description of the textinput field's purpose. For example: "Subscribe
...
Establishes an RDF association between the optional image element [5.4] and this
particular RSS channel.  The rdf:resource's {image_uri}  must be the same as the
...
The URL of the  image to used in the "src" attribute  of the channel's image tag
...
Or the URL to which an HTML rendering of the channel image will link.  This,  as
with the channel's title link, is commonly the parent site's home or news page.
...
particular RSS channel.  Each item's rdf:resource {item_uri} must be the same as
the associated item element's rdf:about {item_uri}.

An RDF Seq (sequence) is used to contain all the items rather than an RDF Bag to
denote item order for rendering and reconstruction.
...
Establishes an RDF association between  the optional textinput element [5.6] and
this particular RSS channel.  The {textinput_uri}  rdf:resource must be the same
as the textinput element's rdf:about {textinput_uri}.
...
The channel element contains metadata describing the channel itself, including a
title,  brief description,  and URL link to  the described resource (the channel
provider's  home  page,  for instance).  The  \{resource\}  URL  of  the channel
element's rdf:about attribute must be unique with respect to any other rdf:about
attributes in the RSS document and  is a URI which identifies the channel.  Most
commonly,  this is either the URL of the homepage being described or a URL where
...
should be of a format supported by the majority of Web browsers. While the later
0.91 specification allowed for a width of 1-144 and height of 1-400,  convention
...
While commonly a news headline,  with RSS 1.0's modular extensibility,  this can
be just about anything:  discussion posting,  job listing, software patch -- any
object with a URI.  There may be a  minimum of one item per RSS document.  While
RSS 1.0 does not enforce an upper limit, for backward compatibility with RSS 0.9
and 0.91, a maximum of fifteen items is recommended.
...
The textinput element affords a method  for submitting form data to an arbitrary
URL  -- usually  located at  the  parent  website.  The  form  processor  at the
receiving end only is assumed to handle the HTTP GET method.

The field is typically  used  as  a  search  box  or  subscription form -- among
others.  While this is  of some use when RSS documents  are rendered as channels
(see MNN) and accompanied by human readable title and description, the ambiguity
in  automatic determination  of meaning  of this  overloaded element  renders it
otherwise not particularly useful. RSS 1.0 therefore suggests either deprecation
or augmentation with  some form of resource discovery of  this element in future
versions while maintaining it for backward compatiblity with RSS 0.9.

\{textinput_uri\} must be unique with  respect to any other rdf:about attributes
in  the   RSS  document   and  is   a  URI   which  identifies   the  textinput.
\{textinput_uri\} should be identical to the  value of the <link> sub-element of
the <textinput> element, if possible.
...
The outermost level in every RSS 1.0 compliant document is the RDF element.  The
opening RDF tag assocaties the rdf:  namespace prefix with the RDF syntax schema
and establishes the RSS 1.0 schema as the default namespace for the document.

While any valid namespace prefix may  be used,  document creators are advised to
consider "rdf:" normative. Those wishing to be strictly backward-compatible with
RSS 0.9 must use "rdf:".
