...
type image = {
  ...
  width : int;
      (** Width of the image in pixels. Maximum value is 144, default value is
          88. *)
  height : int;
      (** Height of the image in pixels. Maximum value is 400, default value is
          31. *)
...
(** [cloud] is an optional sub-element of {!channel}. It specifies a web service
    that supports the rssCloud interface which can be implemented in HTTP-POST,
    XML-RPC or SOAP 1.1.
...
(** [guid] is an optional sub-element of {!item}. "guid" stands for globally
    ...
    present, an aggregator may choose to use this string to determine if an item
    is new.
    ...
    the item, that is, a url that can be opened in a Web browser, that points to
    the full item described by the <item> element. An example:
    ...
    If [permalink] is [false], the guid may not be assumed to be a url, or a url
    to anything in particular. *)

type source = { data : string; url : Uri.t }
(** [source] is an optional sub-element of {!item}. - [data] is the name of the
    RSS channel that the item came from, derived from its <title>. - [url] links
...
(** A {!channel} may contain any number of [item]s. An item may represent a
    ...
    - [title] : The title of the item. - [link] : The URL of the item. - [story]
      : The item synopsis. - [content] : The possible full story ([(_,"")] if
      not present). (Extension of RSS2, see
      ...
      [author] : Email address of the author of the item. - [category] :
      Includes the item in one or more categories. - [comments] : URL of a page
      for comments relating to the item. - [enclosure] : Describes a media
      object that is attached to the item. - [guid] : A string that uniquely
      identifies the item. - [pubDate] : Indicates when the item was published.
...
(** Here's a list of the required channel elements, each with a brief
    ...
    - [language]: The language the channel is written in. This allows
      ...
      page. A list of allowable values for this element, as provided by
      Netscape, is here. You may also use values defined by the W3C. -
      [copyright]: Copyright notice for content in the channel. -
      [managingEditor]: Email address for person responsible for editorial
      content. - [webMaster]: Email address for person responsible for technical
      issues relating to channel. - [pubDate]: The publication date for the
      content in the channel. For example, the New York Times publishes on a
      daily basis, the publication date flips once every 24 hours. That's when
      the pubDate of the channel changes. All date-times in RSS conform to the
      Date and Time Specification of RFC 822, with the exception that the year
      ...
      Follows the same rules as the <item>-level category element. See
      {!category}. - [generator]: A string indicating the program used to
      generate the channel. - [docs]: A URL that points to the documentation for
      the format used in the RSS file. It's probably a pointer to
      [http://www.rssboard.org/rss-specification]. It's for people who might
      stumble across an RSS file on a Web server 25 years from now and wonder
      what it is. - [cloud]: Allows processes to register with a cloud to be
      notified of updates to the channel, implementing a lightweight
      publish-subscribe protocol for RSS feeds. See {!cloud}. - [ttl]: ttl
      stands for time to live. It's a number of minutes that indicates how long
      a channel can be cached before refreshing from the source. - [image]:
      ...
      hours listed in the <skipHours> element. The hour beginning at midnight is
      hour zero. - [skipDays]: A hint for aggregators telling them which days
      they can skip. This element contains up to seven <day> sub-elements whose
...
(** [parse xml] returns the channel corresponding to [xml].
    ...
    Raise [Error.Expected], [Error.Size_Exceeded] or [Error.Item_expectation] if
    [xml] is not a valid RSS2 document. *)
...
(** [to_atom ch] returns an Atom feed that (mostly) contains the same
    ...
    @param self
      the URI from where the current feed was retrieved. Contrarily to Atom,
      RSS2 has no provision to store the URI of the feed itself. Giving this
      information will add an entry to the [links] field of Atom feed with
      [rel = Self]. *)
