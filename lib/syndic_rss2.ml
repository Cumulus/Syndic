...
let image_url_of_xml ~xmlbase (pos, _tag, datas) =
  ...
  with Not_found ->
    raise (Error.Error (pos, "The content of <uri> MUST be a non-empty string"))
...
let image_of_xml =
  let data_producer =
    [
      ("url", image_url_of_xml); ("title", image_title_of_xml);
      ("link", image_link_of_xml); ("width", image_width_of_xml);
      ("height", image_height_of_xml); ("description", image_description_of_xml);
    ]
  in
...
let make_cloud ~pos (l : [< cloud' ] list) =
  ...
  let port =
    ...
    | _ ->
        raise (Error.Error (pos, "Cloud elements MUST have a 'port' attribute"))
  in
  let path =
    match find (function `Path _ -> true | _ -> false) l with
    | Some (`Path p) -> p
    | _ ->
        raise (Error.Error (pos, "Cloud elements MUST have a 'path' attribute"))
  in
...
let cloud_attr_producer =
  [
    ...
    ("path", fun ~xmlbase:_ a -> `Path a); (* XXX: it's RFC compliant ? *)
...
let make_enclosure ~pos (l : [< enclosure' ] list) =
  ...
  let length =
    ...
    | _ ->
        raise
          (Error.Error (pos, "Enclosure elements MUST have a 'length' attribute"))
  in
  let mime =
...
let enclosure_of_xml =
  let attr_producer =
    [
      ("url", url_of_xml); ("length", fun ~xmlbase:_ a -> `Length a);
...
type guid = {
  data : Uri.t;
  (* must be uniq *) permalink : bool; (* default true *)
}
...
let make_source ~pos (l : [< source' ] list) =
  ...
  let url =
    ...
    | Some (`URL u) -> u
    | _ ->
        raise (Error.Error (pos, "Source elements MUST have a 'url' attribute"))
  in
...
let item_of_xml =
  let data_producer =
    [
      ("title", item_title_of_xml); ("description", item_description_of_xml);
      (* <content:encoded> where
         xmlns:content="http://purl.org/rss/1.0/modules/content/" *)
      ("encoded", item_content_of_xml); ("link", item_link_of_xml);
      ("author", item_author_of_xml); ("category", category_of_xml);
      ("comments", item_comments_of_xml); ("enclosure", enclosure_of_xml);
      ("guid", guid_of_xml); ("pubDate", item_pubdate_of_xml);
...
let item_of_xml' =
  let data_producer =
    [
      ...
      ("enclosure", enclosure_of_xml'); ("guid", guid_of_xml');
...
let make_channel ~pos (l : [< channel' ] list) =
  ...
  let link =
    ...
    | _ ->
        raise
          (Error.Error
             (pos, "<channel> elements MUST contains exactly one <link> element"))
  in
  let description =
    match find (function `Description _ -> true | _ -> false) l with
...
let channel_webmaster_of_xml ~xmlbase:_ (pos, _tag, datas) =
  ...
  with Not_found ->
    raise
      (Error.Error (pos, "The content of <webMaster> MUST be a non-empty string"))
...
let channel_generator_of_xml ~xmlbase:_ (pos, _tag, datas) =
  ...
  with Not_found ->
    raise
      (Error.Error (pos, "The content of <generator> MUST be a non-empty string"))
...
let channel_rating_of_xml ~xmlbase:_ (pos, _tag, datas) =
  ...
  with _ ->
    raise
      (Error.Error
         ( pos,
           "The content of <rating> MUST be a non-empty string representing an \
            integer" ))
...
let channel_skipHours_of_xml ~xmlbase:_ (pos, _tag, datas) =
  ...
  with _ ->
    raise
      (Error.Error
         ( pos,
           "The content of <skipHours> MUST be a non-empty string representing \
            an integer" ))
...
let channel_of_xml =
  let data_producer =
    [
      ("title", channel_title_of_xml); ("link", channel_link_of_xml);
      ...
      ("generator", channel_generator_of_xml); ("docs", channel_docs_of_xml);
      ("cloud", cloud_of_xml); ("ttl", channel_ttl_of_xml);
      ("image", image_of_xml); ("rating", channel_rating_of_xml);
      ("textinput", textinput_of_xml); ("skiphours", channel_skipHours_of_xml);
      ("skipdays", channel_skipDays_of_xml); ("item", item_of_xml);
    ]
  in
  ...
  let links =
    ...
    | Some g, _ ->
        (* Sometimes the guid sets [l.permalink = false] but is nonetheless the
           only URI we have. *)
        if looks_like_a_link g.data then
