...
let of_rfc822 s =
  let make_date day month year h m maybe_s z =
    ...
    let date_and_time =
      ...
      in match map Ptime.of_date_time date_and_time with
      | Some x -> x
  ...
  try
    if 'A' <= s.[0] && s.[0] <= 'Z' then (
      ...
      with _ -> sscanf s "%_s %ist %s %i %i:%i%s %s" make_date
    )
    else (
      try sscanf s "%i %s %i %i:%i%s %s" make_date
      with _ -> sscanf s "%i %s %i" (fun d m y -> make_date d m y 0 0 "" "UT")
    )
  with _ ->
...
let to_rfc822 t =
  ...
  Printf.sprintf "%s, %d %s %d %02d:%02d:%02d %04d"
    ds d ms y hh mm ss t
