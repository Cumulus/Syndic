...
let make_person datas ~pos:_ (l : [< person' ] list) =
  ...
  let name =
    ...
    | Some (`Name s) -> s
    | _ ->
        (* The spec mandates that <author><name>name</name></author> but several
           feeds just do <author>name</author> *)
        get_leaf datas
  in
...
let person_data_producer =
  [
    ("name", person_name_of_xml); ("uri", person_uri_of_xml);
...
let rel_of_string s =
  ...
  | "via" -> Via
  | uri ->
      (* RFC 4287 § 4.2.7.2: the use of a relative reference other than a simple
         name is not allowed. Thus no need to resolve against xml:base. *)
...
let link_href_of_xml ~xmlbase a = `HREF (XML.resolve ~xmlbase (Uri.of_string a))
...
let link_attr_producer =
  [
    ("rel", fun ~xmlbase:_ a -> `Rel a); ("type", fun ~xmlbase:_ a -> `Type a);
...
let make_source ~pos (l : [< source' ] list) =
  (* atomAuthor* *)
  let authors =
    List.fold_left (fun acc -> function `Author x -> x :: acc | _ -> acc) [] l
  in
  (* atomCategory* *)
  let categories =
    List.fold_left
...
let source_of_xml =
  let data_producer =
    [
      ("author", author_of_xml); ("category", category_of_xml);
      ("contributor", contributor_of_xml); ("generator", generator_of_xml);
      ("icon", icon_of_xml); ("id", id_of_xml); ("link", link_of_xml);
      ("logo", logo_of_xml); ("rights", rights_of_xml);
      ("subtitle", subtitle_of_xml); ("title", title_of_xml);
...
(* atomInlineTextContent = element atom:content { atomCommonAttributes,
   ...
   atomOutOfLineContent = element atom:content { atomCommonAttributes, attribute
   type { atomMediaType }?, attribute src { atomUri }, empty }
...
let make_entry ~pos l =
  ...
  (* atomSource? *)
  let sources =
    List.fold_left (fun acc -> function `Source x -> x :: acc | _ -> acc) [] l
  in
  let source =
    match sources with
    | [] -> None
  ...
  let authors =
    ...
    | [], Some (s : source) -> (
        ...
        | [] ->
            let msg =
              "<entry> does not contain an <author> and its <source> neither \
               does"
            in
            raise (Error.Error (pos, msg)))
  ...
  let title =
    ...
    | _ ->
        raise
          (Error.Error
             (pos, "<entry> elements MUST contains exactly one <title> elements"))
  in
  (* atomUpdated *)
  let updated =
    match find (function `Updated _ -> true | _ -> false) l with
...
let entry_of_xml =
  let data_producer =
    [
      ("author", author_of_xml); ("category", category_of_xml);
      ("contributor", contributor_of_xml); ("id", id_of_xml);
      ("link", link_of_xml); ("published", published_of_xml);
      ("rights", rights_of_xml); ("source", source_of_xml);
      ("content", content_of_xml); ("summary", summary_of_xml);
      ("title", title_of_xml); ("updated", updated_of_xml);
    ]
  in
...
let make_feed ~pos (l : _ list) =
  ...
  let fix_author pos (e : entry) =
    ...
    | a, [] when a.name = dummy_name -> (
        (* In an Atom Feed Document, the atom:author elements of the containing
           atom:feed element are considered to apply to the entry if there are
           no atom:author elements in the locations described above.
           http://tools.ietf.org/html/rfc4287#section-4.2.1 *)
        match authors with
  ...
  let entries =
    List.fold_left
      (fun acc -> function
        | `Entry (pos, e) -> fix_author pos e :: acc | _ -> acc)
      [] l
  in
  ({
     authors;
...
(* atomFeed = element atom:feed { atomCommonAttributes, (atomAuthor* &
   atomCategory* & atomContributor* & atomGenerator? & atomIcon? & atomId &
   atomLink* & atomLogo? & atomRights? & atomSubtitle? & atomTitle & atomUpdated
...
let feed_of_xml =
  let data_producer =
    [
      ...
      ("logo", logo_of_xml); ("rights", rights_of_xml);
      ("subtitle", subtitle_of_xml); ("title", title_of_xml);
      ("updated", updated_of_xml); ("entry", entry_of_xml);
    ]
  in
...
let set_main_author_entry author (e : entry) =
  (* If the entry has a source, then [author] should be ignored and the one from
     the [source] should be used instead. *)
