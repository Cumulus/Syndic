...
let timer =
  ...
  (fun () ->
    if !already_used
    then Lwt_unix.sleep 1.0 >>= fun () -> already_used := true; Lwt.return ()
    else (already_used := true; Lwt.return ()))

let get : src -> Xmlm.source Lwt.t = function
  | `Uri src ->
     timer ()
     >>= fun () -> CLU.Client.get src
...
let result_equal a b = match a, b with
  | SyndicError (_, err),
    SyndicError (_, err') -> err = err'
...
let print_result p (r, e) =
  ...
  | W3CError errors ->
    (error p "\n%a")
      ...
      errors
  | AnotherError exn ->
    (error p "exn: %s") (Printexc.to_string exn)
  | Ok ->
